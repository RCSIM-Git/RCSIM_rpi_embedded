...
class GlobalPlanner:
    ...
    def __init__(
        ...
        self.inflation_radius_px = int(
            math.ceil(inflation_radius / resolution))
        ...
        while open_set:
            ...
            for dr, dc, base_cost_step in self.directions:
                ...
                if 0 <= neighbor_r < rows and 0 <= neighbor_c < cols:
                    ...
                    added_cost = base_cost_step + \
                        hallway_penalty + terrain_penalty

                    tentative_g_score = g_score[(
                        current_r, current_c)] + added_cost

                    if tentative_g_score < g_score.get(
                            (neighbor_r, neighbor_c), float('inf')):
                        came_from[(neighbor_r, neighbor_c)] = (
                            current_r, current_c)
                        g_score[(neighbor_r, neighbor_c)] = tentative_g_score
                        f_score = tentative_g_score + \
                            heuristic(neighbor_r, neighbor_c)
                        heapq.heappush(
                            open_set, (f_score, neighbor_r, neighbor_c))

            iters += 1
            if iters > max_iters:
                self.logger.error(
                    "GlobalPlanner: A* iteration limit exceeded.")
                break
